...
#                print(rows)
                ...
                # Unit : cm for self.ap_coordinates
                RSSI.append(float(self.ap_coords[int(rows[1])][0])/100.) # location x
                RSSI.append(float(self.ap_coords[int(rows[1])][1])/100.) # location y
                RSSI.append(float(self.ap_coords[int(rows[1])][2])/100.) # location z
                ...
                RSSI.append(int(self.tag_annotations[IDLast_tagAnno][5])) # tag ID  
                ...
                RSSI.append(float(self.tag_annotations[IDLast_tagAnno][4])) # distance between tag and target
                RSSI.append(int(rows[2])) # wearable id
...
#        mydict[-1] = 2
        ...
        with open(filenameStr, "rt") as csvfile:
            ...
            for rows in readerO:
                ...
                # Unit : cm for self.tag_coordinates
                # Unit : m  for self.tag_annotations
                pos_person.append(float(rows[3]) + float(self.tag_coordinates[tagID][0])/100.) 
                pos_person.append(float(rows[4]) + float(self.tag_coordinates[tagID][1])/100.)
                pos_person.append(float(rows[5]) + float(self.tag_coordinates[tagID][2])/100.)

                distance =  (float(rows[3]))**2
                distance += (float(rows[4]))**2
                #distance += (float(rows[5]))**2
                distance = distance**0.5
                pos_person.append(distance)
                pos_person.append(tagID)
                ...
                ## debug
                #print(rows)
                #print("lineID {}, X, pos_relative {} + pos_tag {} = pos {}".format(lineID, float(rows[3]),float(self.tag_coordinates[tagID][0]),float(rows[3]) + float(self.tag_coordinates[tagID][0])))
                #print("lineID {}, Y, pos_relative {} + pos_tag {} = pos {}".format(lineID, float(rows[4]),float(self.tag_coordinates[tagID][1]),float(rows[4]) + float(self.tag_coordinates[tagID][1])))
...
#    h.plot_tags_aps( r"./a")
    ...
    #experiment_dir = r"../../ble-accelerometer-indoor-localisation-measurements/house_B/experiments/fingerprint_activity"
    #experiment_dir = r"../../ble-accelerometer-indoor-localisation-measurements/house_B/experiments/living_1"

    #  House A
    house_dir = r"../../ble-accelerometer-indoor-localisation-measurements/house_A/metadata"
    experiment_dir = r"../../ble-accelerometer-indoor-localisation-measurements/house_A/experiments/fingerprint_floor"
    h = house(house_dir_=house_dir,experiment_dir_=experiment_dir)
    RSSI = h.getRSSIData()

    # write
    f = open("data_step1_rssi_ap.txt","w")
    for k in RSSI:
        line = ""
        line += (str(k) + " ")
        for ele in RSSI[k]:
